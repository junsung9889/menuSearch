...
### 🤖 **AI 자동 페이지 분류** 
- **지능형 분류**: PRIMARY/SECONDARY/ERROR/INFO 4단계 AI 자동 분류
- **규칙 기반 필터링**: 키워드, 계층 깊이, 패턴 매칭으로 1차 필터링 (24.8% 제거)
- **AI 분류 필터링**: ERROR/INFO 페이지 자동 제외 (8.0% 추가 제거)
- **PRIMARY 가중치**: 핵심 페이지에 1.2배 검색 가중치 부여

### 🧠 **RAG 파이프라인 최적화**
- **검색어 최적화**: AI가 "카드 이용 내역" → "카드이용내역"으로 복합어 변환
- **이중 검색**: 원본 + 최적화 검색어로 이중 검색 후 결과 병합
...
```
🎯 자연어 입력 → 🔧 검색어 최적화 → 🔍 이중 하이브리드 검색 → 🧠 AI 컨텍스트 분석 → 🎖️ 최종 추천
```

### 개선된 검색 플로우:
```
0단계(사전준비): AI 4단계 페이지 분류 + 데이터 강화 + 임베딩 최적화
 ↓
입력: "카드 이용 내역 알고 싶어" (자연어 그대로)
 ↓
검색어 최적화: AI가 "카드이용내역 알고 싶어"로 복합어 변환
 ↓
이중 하이브리드 검색: 
  - 원본 검색어로 검색 (1.1배 가중치) 
  - 최적화 검색어로 검색
